...
def registerProcess(request):
    ...
    if request.method == 'POST':
        ...
        if responseData['status'] == 0:
            if not(re.match('^[0]{1}[9]{1}[0-9]{8}$', phone)):
...
def socialLoginProcess(request):
    ...
    if request.method == 'POST':
        ...
        if responseData['status'] == 0:
            if request.POST['google_account'] and request.POST['email']:
                ...
                if cursor.rowcount > 0:
                    ...
                    values = (request.POST['google_account'], request.POST['email'])
                    cursor.execute(sqlQuery, values)
                    conn.commit()
        ...
        if responseData['status'] == 0:
            if request.POST['facebook_account']:
                sqlQuery = 'select * from `hkshopu_user` where `email` = %s limit 1'
                values = (request.POST['email'], )
                cursor.execute(sqlQuery, values)
                if cursor.rowcount > 0:
                    sqlQuery = 'update `hkshopu_user` set `facebook` = %s where `email` = %s'
                    values = (request.POST['facebook_account'], request.POST['email'])
                    cursor.execute(sqlQuery, values)
                    conn.commit()
        ...
        if responseData['status'] == 0:
            if request.POST['apple_account']:
                sqlQuery = 'select * from `hkshopu_user` where `email` = %s limit 1'
                values = (request.POST['email'], )
                cursor.execute(sqlQuery, values)
                if cursor.rowcount > 0:
                    sqlQuery = 'update `hkshopu_user` set `apple` = %s where `email` = %s'
                    values = (request.POST['apple_account'], request.POST['email'])
                    cursor.execute(sqlQuery, values)
                    conn.commit()
